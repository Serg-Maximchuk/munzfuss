...
### W. ✅ Clean up §0z violations surfaced by `scripts/audit_prose.py`  *(opened 2026-05-13, promoted Normal → High 2026-05-23, **closed 2026-09-03**)* *(est: medium-large)* *(type: prose cleanup + linter integration)*

**Closed 2026-09-03. 530 errors → 0**, and `audit_prose` is now a pre-commit hard block on the error tier. Whole corpus went 1169 hits → 154, all warnings. What actually happened, because it diverged from the plan above in three ways worth recording:

  1. **§2 was the wrong rule, not the data.** Measuring first showed the corpus already lived by source-fidelity — of 13 `Münz-` occurrences inside quotation marks, ZERO had ever been re-spelled, while our own prose ran `Müntz-` 102 : `Münz-` 38 and EN/UK prose carried period-form standard names 134 times. The old «mandatory, DE-only» framing declared all three states violations, which is why the linter reported hundreds of §2 errors against correct text. Curator direction 2026-09-02: CLAUDE.md §2 rewritten as **three tiers** (source form untouchable in quotes / titles / URLs / named instruments; standard names identical across languages; period register RECOMMENDED, warning-only, in our own DE prose), plus a tier-1 clause binding a named instrument to its own LANGUAGE — written symmetrically so the planned `da` field needs no re-work. §2 now contributes zero errors by design.
  2. **Most hits were in `verification_note`, which NO template renders** (measured: the «(?)» tooltip is the generic `marker.unverified` i18n string). 935 of 1169. Fixed anyway — it is curator-facing prose a future template could surface — but the reader-impact number was 234 hits / 53 errors, not 1169 / 530.
  3. **The boilerplates needed rewriting, not token edits.** Seven seed-builder note templates (bruun, galster, kmk, ikmk, ucoin, numismaster, hede-stub) were wall-to-wall §0z — pipeline stage names, workflow steps, schema values, internal doc paths, `§BF`/`§BK`/`§AZ` backlog ids, and `spezifikische`, which is not a German word and had been stamped 3546 times. Builders fixed for future seeding; the three existing layers healed by `scripts/maintenance/rewrite_verification_notes.py` (new) — **no re-flow**, so no unrelated cache drift entered the commits.

**Deliberate non-decisions, each measured rather than assumed:**
  - **No rule for digit-form `§` marks.** Of 5039 occurrences of § + digit in `data/`, 4984 sit in monolingual curator fields (`match_uncertainty::why`, decision `reason`, `scope_note`, `_curation_holds`) where CLAUDE.md positively requires a rationale, and ~44 of the 55 in i18n prose are genuine citations to a real decree («§10 der Forordning vom 5. Januar 1813», «Altonaer Bankordnung §19»). A `§\d` rule would have fired four times as often on correct scholarship as on real leaks. Letters-only (`§BF`) is the shape that separates them.
  - **`events.<key>.note` is not linted.** It is a MONOLINGUAL string explaining where an anchor DATE came from — role-1 by design. The structural tell now recorded in the linter docstring: a field carrying a {de, en, uk} triple was built to be read by someone; a plain-string field was not.

**Still open, tracked separately:** 13 fields carrying genuine unsourced hypotheses in rendered prose («Numista/ucoin 1,85 g vermutlich Schreibfehler», «probably struck by Schwabe» from similarity, «wahrscheinlich verwechselt Bobzin …»). Each needs a §0b-1 verification step or a curator call — label as hypothesis, or move from `note` to the non-rendered `verification_note`. Also optional: 123 §2 tier-3 warnings, pure house style, which by the rewritten §2 can never block anything.
...
- **Davenport volume-series — 569, LEGIT (by design).** `dav` «EC II 3529»,
  ...
  GT I», «Dav SG», …) and `lib/numista_canonical.py` lines 99-106
  (`_davenport_volume_ref`) INTENTIONALLY routes «Dav <series>» → `dav` field as
  «<series> <number>». All are Davenport sub-series. **Do NOT touch** — «fixing»
  these would corrupt correct citations.
- **Spaced sub-variants/addenda — 44, LEGIT.** numista/ucoin emit the source's
  literal number with a space: `lange` «762 b», `hede` «39 C», `km` «A 12».
  Cosmetic de-spacing at most; NOT errors.
- **Source year/range/yearbook annotations — 16, LEGIT.** ucoin `schou`
  «44-45 (1694)», galster `jensen_skjoldager` «F-51 - F-58» / «T-81 - T-88»
  ranges, galster `sieg` «[2018] 18» (Sieg yearbook edition). Leave.
- **Galster UU sub-series — 11, MINOR.** numista `galster` «UU 167» / «UU# 165»:
  ...
  tidy at most — NOT foreign.

**The GENUINE issues (~30, the only actionable part):**

1. **ucoin `km` «UC# N» (12)** — ucoin's OWN internal id misfiled into `km`
   («UC# 1» is not a Krause number). → route to `others` or drop. Low-authority
   source; low blast radius.
2. **Bare-dash empties (6)** — hede `sieg` «-», bruun `schive`/`jensen_skjoldager`
   «–». Drop (null markers, not refs). Parser should suppress.
3. **bruun / galster parser garbage (~10, «OTHER review»)** — bruun
   ...
   parser-capture bugs — fix in the respective parser, not a blind data edit.
4. **«var.» suffix (6)** — numista `lange` «16b var.» / «271 var.» / «358 C IV
   var.» / «399 A var.» (anti-pattern #5). Policy call: strip to base index, or
   `others`? «358 C IV» / «399 A» carry extra annotation.
5. **«; unik» annotation (2)** — galster `schou` «13; unik» / «3; unik» — strip
   the «; unik» (unique) annotation, keep the Schou number.

**LESSON (§0b, recorded so the trap isn't re-sprung):** «contains whitespace» is
NOT a reliable «bad index» signal — Davenport volume codes, catalogue sub-letters,
...
**✅ MATERIALISED via the 2026-06-17 coordinated re-flow** (commits `68fae2f` merge + `968af94` absorb). bruun-3839 → 1496-1497, km-795 → 1874-1905 discrete, c9h16a → 1874-1905; the per-case `_curation_holds` on bruun-3839 + km-795 + c7h13a were retired (the mute + §CW resolve them natively). **Extended to a NEW class — ND-swallow:** the re-flow's curation-loss gate caught 4 coins where an UNDATED Bruun specimen's broad «ND (1670-99)»-type range (year_verified=False) was swallowing the dated discrete years of its merged cluster (dk-bruun-10497/10501/10509 in danish_norway, dk-bruun-14741 in gottorp_duchy). Same `year_demote` mechanism applied → they resolve to the dated peers' years (10497 → 1673,1675,1679; 14741 → 1531). The reign-window AND undated-specimen-broad-range cases are both the same loose-span-vs-discrete pollution; year_demote covers both. **✅ Edge case RESOLVED at the root (2026-06-17, commits `8571258` fix+tests / `58bc460` data).** The c7h13a missing-1798 was NOT a union bug but a Hede-seed-builder one: `_build_year_fields` collapsed a consecutive year run (13C «1798, 1799») into a loose `year_first/year_last` span (it emitted `year_ranges` only when gaps existed, `len(ranges)>1`), which the merger could displace under the cluster's [1795,1801] discrete envelope, dropping the only attestation of 1798 (1799 survived via 13D's independent singleton). Fix: emit ONE singleton `[y,y]` per attested year whenever `len(yrs)>1` so the merger classifies Hede years DISCRETE; `_format_year_label` folds them back to «1795-1799, 1801» for display. Materialised via re-seed hede (3 entities, year-only diff) → re-merge → re-absorb (curation-loss gate REAL-LOSS=0, 4 benign year-add). Bonus: three other unified labels self-corrected (nf3h69 1649-1668 → discrete gaps revealed; c5h126a/c5h107 1693-1696 → 1693-1694,1696; c8h3a 1840-1848 → discrete) — the now-discrete Hede years let the merger displace masking ucoin/Numista loose spans, so the page stops asserting un-struck in-between years (§3a/§4). Tests: `TestHedeDiscreteYearEnumeration` + `TestC7H13ASpeciedalerInteriorYear` in `tests/test_union_year_ranges.py` (58 green). Rendered + verified on denmark + schleswig_holstein de/en/uk.
...
**✅ MATERIALISED via the 2026-06-17 coordinated re-flow.** The raw-cache→parsed-sidecar gap was the missing link: build_numista_seed reads `numista/parsed/<NID>.json`, so the 218 discretes (in the raw cache from §CX) had to be backfilled into the parsed sidecars first (220 sidecars, submodule `c18232a7d`). Then re-seed (`3486bf0`) → merge (`68fae2f`) → absorb (`968af94`) carried them through: e.g. Albertsdaler c7h22ab → 1781,1784,1786,1796; 5567 → 14 discrete years. Rendered (year column splits `year_label` by comma). §CX closed.
