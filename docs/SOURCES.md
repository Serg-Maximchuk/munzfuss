...
*Fix (this pass).* Per-coin `_source_errata` on the bruun seed: `metal` gold→silver + drop the phantom `friedberg: 230`; reclassified → `9_25_thaler`/I/kurant (sibling 1704 Speciedalers km-480-2 / dk-tid-94244). The seed edit propagates through merger+absorb immediately. **NOT fixed (curator direction 2026-07-17): the Fr→gold metal-inference rule itself, and the parser-level adjacent-lot Friedberg bleed in `02_parse_lots.py`.** A future coordinated bruun re-seed will REVERT this (wholesale-write builder does not preserve `_source_errata`) UNLESS the parser bleed is fixed first — re-apply the errata or fix the bleed at re-seed time. **Open scope question:** how many other bruun lots carry a bled Friedberg that silently gilded a silver coin — worth a sweep (`metal=gold` bruun seeds whose lot body_excerpt lacks «Fr-»).
