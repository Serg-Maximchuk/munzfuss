...
### D48 — A `merges:` entry names the WHOLE intended class, never the minimal pair

- **Decision (2026-07-27)**: When authoring a `merge_decisions/<entity>.yml::merges` entry, `members` MUST enumerate every seed of the intended equivalence class — including seeds already grouped together — not just the newcomer plus the group's anchor. Encoded as a Step-2 rule + a hard rule in the `v2-merge-coins` skill.
- **Rationale**: `UnionFind.force_union` (`merge_seeds_cross_source.py` ~line 2369) **clears AUTO no_merges** between the two classes it joins — «a curator merge outranks any heuristic». A member that arrives by *transitive auto-join* receives no such clearance, so any auto `no_merge` touching it still vetoes the union. Consequence: a minimal-pair decision can **EXPEL an existing member** of the very group it was meant to enrich. Observed the same day: `[dk-hede-nc5h6, dk-bruun-10509]` threw `dk-tid-145745` out of the ½-Dukat Christiania group — the ucoin record publishes only `km 103`, the Hede page publishes no KM, so their register intersection is empty, the soft weight tier fired a `no_match`, and PASS 2 registered it as an auto `no_merge`. Naming all four members fixed it in a single pass.
- **Why this matters beyond the one case**: it is the cheap, durable lever for the whole zero-shared-register defect class that also motivates the (still unshipped) evidence gate of `00318b8` — Hede pages publish `hede` + `schou` and no Krause number while NumisMaster / ucoin publish only a Krause number, so the two share nothing yet match on metal + nominal + ruler + year. Measured corpus-wide in `00318b8`: 14116 of 134306 confident pair-verdicts (10.5 %) rest on an empty register intersection. Enumerating members is three YAML lines, needs no code change, and stays correct after the matcher fix lands.
- **Companion measure**: for the inverse direction (a zero-overlap record being *attracted* to the wrong class) the surface is `no_merges` — used the same day to keep `km 118` records with Hede 3 and to protect the curated KM 119 split from `dk-hede-nc5h8`. With those two fronts closed, `nc5h8` auto-joined its real partner `dk-bruun-10498` unaided, on the two registers Bruun prints (`hede 8` + `schou 15`).
- **Encoded in**: `.claude/skills/v2-merge-coins/SKILL.md` (Step 2 blockquote + Hard rules); worked example in `data/v2/merge_decisions/danish_norway.yml` (the `nc5h6` four-member entry carries the explanation inline).
