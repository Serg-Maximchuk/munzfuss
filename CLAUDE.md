...
>
> **Rules vs procedures split.** `CLAUDE.md` carries IMMUTABLE RULES — what is forbidden, what is required, what conventions apply. `docs/PLAYBOOKS.md` carries EXECUTABLE PROCEDURES — how to actually do recurrent tasks (per-case Hede dedup merge, ucoin Composition harvest session, web-research citation cycle, Müntzfuß disambiguation, adding a Bruun specimen, adding a new location, cross-Krause-volume KM-collision handling, session lifecycle, closing a TODO, removing a coin from the render). When you find yourself asking «what's the procedure for X?», check `docs/PLAYBOOKS.md` PB-N first.
>
...
- **Catalogue index numbers written into the prose** — «Hede f2h3», «Galster 131», «KM 138.2», «Sieg 102.1», a one-issue mintmaster («Mzm. Jørgen Kock»). Naming a specific catalogue entry (or its mint-master) in the description text is issue/specimen-level detail; it belongs on the coin row. The attestation still reaches the reader through the `<sup>[ref:KEY]</sup>` citation — whose `refs_pool.yml` body MAY name the catalogue number as a §5a locator — while the prose names only the standard, the ruler, the period and the fineness step. A catalogue number in the prose is forbidden; the same number inside a ref body is a correct citation. (Curator direction 2026-07-03, enforced by the `fuss-description` skill's C6 + `describe_helper.py` CATALOGUE/MINTMASTER scan. This tightens the earlier reading that treated catalogue TYPE numbers as neutral attestation — that reading holds for coin-row attestation, NOT for the system-level Fuß description prose.)
...
4. **Duplicates and sub-variants — decided by the catalogue-index GRAPH, not by a single index difference.** A true **duplicate** is two entries carrying the **same FULL index** (both `KM 138.2`, both `Hede 39A`) for the same physical type — these collapse into ONE entry per §9a (merge every observation, never drop). But a bare index *difference* does NOT by itself make two entries distinct types: the decision needs the whole index graph (procedure below).

   **Sub-indices are sub-variants of ONE coin, not separate types.** A base index plus a sub-suffix — `KM 138.1 / 138.2 / 138.3`, `Hede 39A / 39B / 39C`, `Sieg 102.1 / 102.3` — denotes design / mint / mintmaster / die / fineness *variations within one base type*. The base number (`KM 138`, `Hede 39`, `Sieg 102`) **is** the type; the sub-suffix is the variation. Same base + different sub-suffix is a **sub-variant-to-merge** into one row, NOT a distinct type. (We tripped over this repeatedly — the old «different index = different type» heuristic was too blunt; this is the corrected rule, curator direction 2026-06-28.)

   **Catalogues enumerate variants at different granularity, and they disagree with each other.** One catalogue may split a coin into `.1 / .2 / .3` sub-variants while another groups the very same pieces under a single number (with or without letter sub-tags), and a third assigns them unrelated base numbers (Krause numbering restarts per volume). The enumeration divergence is NOT evidence of distinct types — it is just different cataloguing granularity. Concrete: `KM 138.1–138.5` (five Krause sub-variants) ARE `Hede 39` (one Hede number with letters 39A–39G) — one coin.

   **Operational rule — build the index graph, look for a unifying catalogue.** Before declaring two candidate entries «same coin» or «different type», collect EVERY catalogue index each entry carries across ALL its sources (KM, Hede, Sieg, Schou, Dav, Lange, Fr, …) into one graph, then:
   - If **any** catalogue assigns the candidates a single base index (with or without sub-variations) → they are **ONE coin**. Merge into one entry per §9a, preserving every sub-index in list-form (`km: [138.1, 138.2, 138.3, 138.5]`, `hede: [39, 39A, 39B]`) so no provenance is lost. When the unification spans Krause VOLUMES (a Danish KM and a German KM of the same Hede coin), keep the volume provenance distinct via dict-form `km` so the two Krause traditions are not conflated (the c5h125 / c7h13 pattern).
   - They are **distinct types** (separate rows) only when EVERY catalogue assigns them distinct BASE indices that no catalogue unifies — e.g. `KM 66 = Hede 118` (1619) vs `KM 67 = Hede 119` (1619–1623): both Krause AND Hede separate them → two coins, two rows (the c4h119 case).
   - **When the graph is contested** (a catalogue is missing for one candidate, or the sources genuinely disagree on the base) → escalate to the user per §0/§8a, don't guess.

   **Caveat — same bare numeric index across different catalogue volumes is NOT automatically the same coin** (the inverse trap). Krause-Mishler numbering restarts within each country / region: `KM#75` in the «Denmark» Krause volume is an entirely different coin from `KM#75` in the «German States — Schleswig-Holstein» volume. Likewise Hede (Danish), Sieg (Danish-Norwegian), Lange (Holstein), Behrens (Lübeck) — each catalogue's numbering is internal to that catalogue. Two entries colliding on a bare numeric `km`, `hede`, `lange` etc. unify **only if** they refer to the SAME catalogue scope (same country/region in Krause; same author in the regional catalogues). When building the index graph, always pre-screen by composition + nominal + ruler + year before treating a bare-number collision as a unifying edge; if those don't line up, the «same» number is just a numeric coincidence between two unrelated catalogue volumes (which is why the graph needs a *real* unifying catalogue, not an accidental number clash).

   **Two further caveats on edge STRENGTH, both documented in `docs/SOURCES.md` §13.12.** (a) *Editions renumber.* Sieg's 2001 catalogues permuted the whole Christian VII reign, and our two suppliers of Sieg numbers sit on opposite sides of it — so two Sieg numbers ~30 apart on a Christian VII coin are the same type in two systems, not two types, and a bare agreement may be a false edge. Verified n = 25, zero exceptions. Until the residual there is resolved, **Sieg is not usable as the load-bearing key for Christian VII** — use Hede or Schou. Hede itself has 1964 / 1971 / 1978 printings with changed numbering. (b) *Catalogues are not independent witnesses.* Sieg → Hede → Schou is a derived chain (Sieg's principal source is Hede; Hede's systematics rest substantially on Schou), so agreement between a catalogue and the one it derives from is ONE reading propagated twice, not corroboration. An edge from a derived catalogue is weaker than the same edge from its source, and a mid-chain error travels downstream.
...
**Do NOT skip:**
- Multiple coins of the same denomination per ruler when they carry distinct BASE catalogue indices that **no** catalogue unifies (each distinct base type = its own row). Sub-variants of one base (`KM 138.1` vs `138.2`, unified by `Hede 39`) are NOT separate rows — they merge into one entry per item 4.
...
**Applying an exclusion to a coin already in the data — the surface is `data/v2/exclusions/<entity>.yml`, and a parser filter is NOT a substitute.** Tightening a source filter (`PATTERN_RE` in a parser, an `_excluded_*` rule in a seed builder) only stops the builder EMITTING the entry. It does NOT remove one that is already in the seed: `merge_seed` deliberately preserves entries the parser no longer produces (`orphan_curated`, appended to the seed's tail) so that no filter change can silently drop curated data. So a filter-only fix re-runs the entire pipeline and changes NOTHING in the render — which reads as «the fix failed» when it behaved exactly as designed. The two changes serve different purposes and a real exclusion usually needs BOTH: the `exclusions/` entry removes the coin that is here, the filter stops a future re-harvest re-introducing it. Full procedure, including the verification that the off-strike's mother coin survives untouched: `docs/PLAYBOOKS.md` PB-12. (Learned the hard way on `dk-bruun-7396`, 2026-07-31.)
...
**Reconciliation NEVER replaces `sources` — always UNION.** When a manual final reconciliation copies fields from a seed_unified cluster after a merger run, the `sources` list MUST be unioned, NEVER overwritten: `final.sources = union(final.sources, cluster.sources)`, never `final.sources = cluster.sources`. The cluster is NOT a superset of the final's prior sources — the final may carry citations from earlier dedup folds (e.g. a `km-651-1` Bruun lot folded into `c7h13a`) or prior curator additions that the current cluster doesn't re-attest. A blind replace silently drops those (caught 2026-06-28: c7h13 lost «Bruun Part III, lot 11295» — the SOLE source for KM 651.1; a sweep then found 5 such finals). The normal absorb path is safe (`_enrich_final_entry` → `_collect_sources` unions); this rule binds the HAND-WRITTEN reconciliation path. Same union-not-replace discipline applies to `weight_rough_g`, `fineness`, `diameter_mm`, `catalog` (use the merger's `_collect_*` / `_deep_merge_catalog` helpers, or `dedup_final_foundations.fold` which already unions). Guards: `scripts/maintenance/audit_lost_citations.py` (detector + `--apply` healer; wired into `.githooks/pre-commit` Check 6, hard-block when a `data/v2/final/*.yml` is staged).
...
- **`.githooks/pre-commit`** — seven checks, run only when the commit touches the relevant files: (1) `build.py --validate-only` HARD BLOCK; (2) `audit_prose.py --staged` HARD BLOCK on ERRORS, advisory on warnings (promoted 2026-09-03 once §W took the corpus to 0 errors — the error tier is §0z/§0a role-3 leaks and §2a fabricated non-words, none of which has a legitimate use; the WARNING tier will NOT be promoted, because all of §2 lives there and §2 tier 3 is house style, which by its own wording is never grounds to refuse a commit); (3) `audit_i18n.py` advisory; (4) `audit_v2.py --quick` (I1/I2/I3/I5/I8) HARD BLOCK on staged `data/v2/**`; (5) `validate_decisions.py` structural + `--check-members` (member-resolution) HARD BLOCK on staged `merge_decisions/**`; (6) `audit_lost_citations.py` HARD BLOCK on staged `final/**`; (7) `verify_reflow.py` HARD BLOCK on losses against HEAD on staged `final/**`. **Install once per clone via `./scripts/install_hooks.sh`** — sets `git config core.hooksPath .githooks`. Bypass per-commit with `git commit --no-verify`. See `.githooks/README.md` for what blocks and why.

## Skills (executable procedures)

Project skills live in `.claude/skills/<name>/SKILL.md` (version-controlled — the `.gitignore` un-ignores `.claude/skills/`). Invoke via the Skill tool / `/<name>`. They are the EXECUTABLE form of recurrent procedures (the prose lives in `docs/PLAYBOOKS.md`); prefer them over re-deriving a flow by hand — they bake in the guards learned from real failures.

- **`v2-merge-coins`** — merge OR split coins in the V2 pipeline via `merge_decisions`. Use for «роби мердж», «це over-merge», «почисти orphan-и». Enforces the two guards that prevent the recurring failures: (a) the **§9.4 over-merge gate** — `merge_helper.py graph <entity> <ids>` flags candidates with NO shared catalogue base (merged on ruler+nominal+year alone — the gottorp KM33/Lange274b class); (b) **seed-id resolution** — `merge_helper.py resolve <entity> <id>` maps any render/final/foundation id to the SEED id a `merge_decision` requires. Members are SEED ids OR a bare Hede code (the merger's `_expand_member` expands `dk-hede-c4h112` → `c4h112a/c4h112b`, grouping them so a `no_merges` never blocks the within-coin pair — NEVER re-point a bare Hede code to a flat sub-variant list). Covers the foundation-reset trap (a curated `km-NN-…` final whose stored catalog re-grabs a member after a split). Executable form of PB-1.
- **`v2-audit`** — READ-ONLY pipeline health review. Use for «перевір пайплайн», «є over-merge десь?», «health check». Orchestrates: member-resolution (`validate_decisions.py --check-members`), the **over-merge scan** (`merge_helper.py scan <entity>` — proactively finds the gottorp class instead of by chance), `audit_v2.py` invariants, `audit_lost_citations.py`, `audit_health.py`. Surfaces findings + routes each to its fix; it never edits data. Remediation is `v2-merge-coins`, not this skill.
- **`fuss-description`** — write / revise / SCORE a Müntzfuß description (`data/shared/fuesse.yml::<fuss>.description`, de/en/uk) against a fixed **6-criterion rubric → X/10**, loopable until it clears a threshold (default 8+). Use for «оціни опис стопи», «напиши/перепиши опис стопи», «покращ опис стопи до 8+». The rubric is the executable form of §7a (system-not-specimens) + §0 (no invention) + §5 (sourcing) + §0z (reader voice): C1 founding (how/why/by whom) · C2 role/function · C3 per-phase differentiator · C4 no metric-fixation (metrics live in Grundwerte) · C5 every claim sourced · C6 no specimen mentions. Helper `describe_helper.py <fuss>` surfaces candidate violations (bare metrics, specimen intrusions, uncited claims, unresolved refs) — candidates, NOT verdicts; the score is agent judgment after source-verification (§0b). **Never invent to raise the score** — an unsourceable fact stays a labelled gap and caps the score.
- **`research-dossier`** — write, extend, and SCORE a `docs/research/<topic>.md` dossier against a fixed **6-criterion rubric → X/10**, loopable (default threshold 8+). Use for «створи документ про цю стопу», «напиши дослідницький документ», «онови досьє», «оціни повноту документа». Executable form of `docs/research/README.md` + §0 / §0b / §0b-1 / §5a / §0z: C1 scope + exclusions stated · C2 parameters complete per period (a gap the sources cannot fill is written as a gap, never inferred) · C3 every change tied to a **dated, named instrument** · C4 provenance chains named (who reproduced whom — the Galster-rework-of-Wilcke class; derived catalogues are not independent witnesses) · C5 open questions listed WITH what would settle each · C6 figures independently recomputed and our own data inventoried against the source. Helper `dossier_helper.py <path>` surfaces candidate violations. SCORE is READ-ONLY. Same hard rule as `fuss-description`: **never invent to raise the score** — an unavailable parameter stays a labelled gap.
- **`merge-candidate-table`** — READ-ONLY. Build a field-by-field comparison table between a coin RECORD under examination (📍) and its merge CANDIDATE(s) (🎯) BEFORE executing a merge. Use for «порівняй ці монети», «побудуй таблицю порівняння», «таблиця мердж-кандидатів», «record vs merge-candidate», «is this the same coin as». Marks each field ✅ match / ⬜ missing (record just poorer — NOT a disqualifier) / ❌ conflict (a real ❌ on a verified field kills the merge), names the **load-bearing catalogue key** that justifies the merge per §9.4 (never nominal+ruler+year alone — that's the over-merge trap), sub-index-in-range counts as ✅ (Schou 3 ∈ 3-4), and lists source links for both sides clearly separated (📍 record vs 🎯 candidate). The read-only evidence step that feeds a curator verdict; hands off execution to `v2-merge-coins` (same-entity or cross-entity). Never edits data.
